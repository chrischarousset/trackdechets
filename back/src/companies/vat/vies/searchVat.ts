...
import { CountryCodes } from "../../../common/constants/companySearchHelpers";
...
export const client = async (
  ...
  if (!(<string[]>Object.values(CountryCodes)).includes(countryCode)) {
  ...
  const { isValid: isRegexpValid } = checkVAT(vatNumber, countries);
  if (!isRegexpValid) {
    throw new UserInputError(
  ...
  try {
    ...
    return {
      ...
      codePaysEtrangerEtablissement: countryCode
    };
  } catch (err) {
